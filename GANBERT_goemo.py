...
# Load GoEmotions CSV dataset

def get_goemo_examples(input_file):
  examples = []
...
def get_goemo_labeled_unlabeled(args,input_file):
  df = pd.read_csv(input_file, names=goemo_column_names, encoding='latin-1')
  ...
  for text, polarity in zip(unlabeled['text'], unlabeled['polarity']):
    unlabeled_examples.append((text, str("4_4")))

  return labeled_examples, unlabeled_examples

#Load goemo examples
goemo_labeled_examples, goemo_unlabeled_examples = get_goemo_labeled_unlabeled(args,goemo_labeled_file)
goemo_test_examples = get_goemo_examples(goemo_test_file)
...
def get_label_map():
  label_map = {}
  for (i, label) in enumerate(goemo_label_list):
    label_map[label] = i
...
train_examples = goemo_labeled_examples
#The labeled (train) dataset is assigned with a mask set to True
train_label_masks = np.ones(len(goemo_labeled_examples), dtype=bool)
#If unlabel examples are available
if goemo_unlabeled_examples:
  train_examples = train_examples + goemo_unlabeled_examples
  #The unlabeled (train) dataset is assigned with a mask set to False
  tmp_masks = np.zeros(len(goemo_unlabeled_examples), dtype=bool)
...
test_label_masks = np.ones(len(goemo_test_examples), dtype=bool)

test_dataloader = generate_data_loader(goemo_test_examples, test_label_masks, label_map, do_shuffle = False, balance_label_examples = False)
...
discriminator = Discriminator(input_size=hidden_size, hidden_sizes=hidden_levels_d, num_labels=len(goemo_label_list), dropout_rate=out_dropout_rate)
...
for epoch_i in range(0, num_train_epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        label2one_hot = torch.nn.functional.one_hot(b_labels, len(goemo_label_list))
